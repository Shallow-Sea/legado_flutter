...
本项目重构过程中参考了以下两个 Legado 衍生项目：

- [LegadoTeam/Legado-Tauri](https://github.com/LegadoTeam/Legado-Tauri) — Legado 的 Tauri (Rust + 前端) 重构版
- [HapeLee/legado-with-MD3](https://github.com/HapeLee/legado-with-MD3) — Legado 的 Material Design 3 fork
...
```bash
# iOS Debug 构建（模拟器）
bash build_ios_debug.sh simulator

# iOS Release 构建（真机，需要 Apple Developer 证书）
...
# iOS (需要 macOS，确保已安装 Rust iOS 目标)
rustup target add aarch64-apple-ios x86_64-apple-ios aarch64-apple-ios-sim
cd core
IPHONEOS_DEPLOYMENT_TARGET=13.0 cargo build -p bridge --target aarch64-apple-ios --release
cd ..
cd flutter_app
flutter run -d ios
...
| 跨平台支持 | 🚧 部分 | Android 全功能可用；iOS 基础设施已就绪（Rust 静态库编译通过、Xcode 项目结构完整、模拟器/真机双目标） |
